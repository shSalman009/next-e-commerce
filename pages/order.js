...
export default function Orderr({ data }) {
  const { orderId, createdAt, products, amount, email } = data;
...
export async function getServerSideProps({ query }) {
  ...
  const details = await Order.findOne({ id: query.id });

  return {
    props: { data: JSON.parse(JSON.stringify(details)) },
  };
}
